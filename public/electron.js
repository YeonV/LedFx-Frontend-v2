...
const ready = () => (
  app.whenReady().then(async () => {
    ...
    const integratedCore = (process.platform === 'darwin')
      ...
      : fs.existsSync(path.join(path.dirname(__dirname), isDev ? 'extraResources' : '../extraResources','LedFx_core.exe'))
    ...
    if (integratedCore) {
      if (process.platform === 'darwin') {
        ...
        subpy = require('child_process').spawn(`${path.join(path.dirname(__dirname), isDev ? 'extraResources' : '../extraResources','LedFx_core.exe')}`, ['-p', '8888', '--no-tray']);
      }
    } 
      ...
      if (parameters === 'start-core') {
        if (integratedCore) {
          if (process.platform === 'darwin') {
            ...
            wind.webContents.send('fromMain', ['currentdir', integratedCore, fs.existsSync(path.join(path.dirname(__dirname), isDev ? 'extraResources' : '../extraResources','LedFx_core.exe'))]);
            subpy = require('child_process').spawn(`${path.join(path.dirname(__dirname), isDev ? 'extraResources' : '../extraResources','LedFx_core.exe')}`, []);
          }
        }
        return;
      }

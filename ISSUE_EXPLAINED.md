...
-   **Atomic Update**: Modified `renameVisualizerInstance` in `storeVisualizerConfigOptimistic.ts` to perform both the optimistic store key rename and the `clientIdentity.name` update in a single atomic operation. This prevents the state from becoming inconsistent during the transition.
-   **Unified Instance Key**: In `VisualizerConfig.tsx`, introduced `resolvedInstanceKey` which prioritizes the local `clientIdentity.name` when the card represents the current client. This prevents the `useEffect` from re-creating the "old" name key if it receives a stale name prop from the backend's client list before the rename has fully propagated through the WebSocket network.
-   **Proactive Cleanup**: Added a cleanup mechanism in `VisualizerConfig` that explicitly deletes any optimistic store entry matching the (stale) `name` prop if it's confirmed to be the local client. This catches any race conditions where the old name might have been re-created before the rename settled.
-   **Safety Delay**: Introduced a 100ms delay in `ClientEdit` between the local optimistic rename and the global `updateClientIdentity` call. This gives the local React components time to process the state change before the backend broadcast loop begins.

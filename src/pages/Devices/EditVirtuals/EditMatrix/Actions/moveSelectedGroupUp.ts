// src/pages/Devices/EditVirtuals/EditMatrix/Actions/moveSelectedGroupUp.ts
import { clone } from '../M.utils'
import { IMoveGroup } from './interfaces'

const moveSelectedGroupUp = ({ m, selectedGroup, setError, setM }: IMoveGroup) => {
  const groupPixels = []
  ...
  for (let i = 0; i < m.length; i++) {
    for (let j = 0; j < m[i].length; j++) {
      if (m[i][j].group === selectedGroup) {
        if (i - 1 < 0) {
          // Part of the group is at the top edge, cannot move up.
          console.log('Move aborted: group is at the top edge.')
          return
        }

    ...
    for (let i = 0; i < inputArray.length; i++) {
        if (inputArray[i].deviceId !== '') {
            if (deviceId === '') {
                ...
                flip = false;
            } else if (inputArray[i].deviceId === deviceId && Math.abs(inputArray[i].pixel - (endPixel as number)) === 1) {
                endPixel = inputArray[i].pixel;
...
export function reverseProcessArray(outputArray: OutputType[], rows?: number): InputType[][] {
    ...
    let finalArray: InputType[][] = [];
    let group: string = '0-0';

    for (let i = 0; i < outputArray.length; i++) {
        let [deviceId, startPixel, endPixel, flip] = outputArray[i];
        if (deviceId === 'gap' || deviceId === 'GAP') {
            for (let j = startPixel; j <= endPixel; j++) {
                inputArray.push({ deviceId: '', pixel: j, group: '0-0' });
            }
        } else {
            if (flip) {
                for (let j = endPixel; j >= startPixel; j--) {

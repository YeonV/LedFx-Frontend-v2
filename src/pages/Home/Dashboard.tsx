...
import { DeleteForever, GitHub, MenuBook, Lightbulb, MoreHoriz } from '@mui/icons-material'
...
/**
 * One source for the six external links, so the desktop Fab row and the phone's
 * overflow menu can never drift apart. On a phone six unlabelled icons - two of
 * which are the same GitHub mark - are a guessing game; the menu gives them names.
 */
const links = [
  { label: 'Core Github', icon: <GitHub />, url: 'https://github.com/LedFx/LedFx' },
  {
    label: 'Core Docs',
    icon: <MenuBook />,
    url: 'https://docs.ledfx.app/en/latest/developer/developer.html'
  },
  { label: 'Client Github', icon: <GitHub />, url: 'https://github.com/YeonV/LedFx-Frontend-v2' },
  { label: 'Client Docs', icon: <MenuBook />, url: 'https://docs.ledfx.stream' },
  { label: 'Type Docs', icon: <SiTypescript />, url: 'https://typedoc.ledfx.stream/' },
  { label: 'Discord', icon: <DiscordIcon />, url: 'https://discord.gg/nyMeUR8C' }
]

const openLink = (url: string) => window.open(url, '_blank', 'noopener,noreferrer')

const linkFabSx = (theme: any) => ({
...
const Dashboard = () => {
  ...
  const smallHeight = useMediaQuery('(max-height: 680px) and (min-width: 480px)')
  const xSmallHeight = useMediaQuery('(max-height: 580px)')
  // Mirrors the `hideTablet` class the gauges use - below this they are gone and
  // the phone would otherwise get no status at all.
  ...
  const devicesOnline = filterDevDevices(devices).filter((key) => devices[key].online)

  const virtualsReal = filterDevDevices(virtuals).filter((key) => !virtuals[key]?.is_device)
      ...
      <Stack spacing={[0, 0, 2, 2, 2]} alignItems="center">
        {!xSmallHeight && (
          <>
            ...
            <Stack spacing={2} direction="row" className="hideTablet">
              <Gauge unit="Pixels online" total={pixelTotal} current={pixelTotalOnline} />
              <Gauge
                unit="Devices online"
                total={Object.keys(devices).length}
              ...
              <Gauge
                unit="User Colors"
                total={
                  ((config.user_colors && Object.keys(config.user_colors)?.length) || 0) +
                  ((config.user_gradients && Object.keys(config.user_gradients)?.length) || 0)
                }
                current={
                  ((config.user_colors && Object.keys(config.user_colors)?.length) || 0) +
                  ((config.user_gradients && Object.keys(config.user_gradients)?.length) || 0)
        ...
        {/* The gauges are `hideTablet`, so under 960px the page showed no numbers
            at all. Same six figures, one line, no 900px of circles. */}
        {isTablet && (
          <Stack
            useFlexGap
            direction="row"
            ...
            sx={{ flexWrap: 'wrap', px: 2, pt: 1 }}
          >
            <Chip
              size="small"
              variant="outlined"
              label={`${pixelTotalOnline} / ${pixelTotal} Pixels`}
            />
            <Chip
              size="small"
              variant="outlined"
              label={`${devicesOnline.length} / ${filterDevDevices(devices).length} Devices`}
              onClick={() => navigate('/Devices')}
            />
            <Chip
              size="small"
              variant="outlined"
              label={`${virtualsReal.length} Virtuals`}
              onClick={() => navigate('/Devices')}
            />
            <Chip
              size="small"
              variant="outlined"
              label={`${Object.keys(scenes).length} Scenes`}
              onClick={() => navigate('/Scenes')}
            />
          </Stack>
        )}

        {/* `.Content` drops its margin below 580px, so an unpadded SmartBar sat
            flush against both edges with its legend clipped. */}
        <Box sx={{ width: '100%', maxWidth: { xs: '100%', sm: 500 }, px: { xs: 2, sm: 0 } }}>
          <SmartBar direct />
        </Box>
        <Stack spacing={2} direction={smallHeight ? 'row' : 'column'} alignItems="center">
          {/* Wraps for the same reason the links row does - at 412px the six
              action fabs overflowed the viewport on both sides, leaving the
              first and last one half off-screen and their centers untappable. */}
          <Stack spacing={2} direction="row" justifyContent="center" sx={{ flexWrap: 'wrap' }}>
            ...
            <Tooltip title="Scan for WLED Devices" sx={{ '&': { marginLeft: '0 !important' } }}>
              <Box
                sx={{
                  m: 0,
                  position: 'relative',
              ...
              >
                ...
                >
                  {scanning > -1 ? (
                    <Typography variant="caption" style={{ fontSize: 10 }} component="div">
          ...
          {/* On a phone six unlabelled icons - two of them the same GitHub mark -
              are a guessing game and eat a whole row. One overflow fab opens the
              same six as a named menu. */}
          {xsmall ? (
            <>
              <Tooltip title="Links & Docs">
                <Fab
                  aria-label="links"
                  onClick={(e) => setLinksAnchor(e.currentTarget)}
                  style={{ margin: '8px', zIndex: 0 }}
                  sx={linkFabSx}
                >
                  <MoreHoriz />
                </Fab>

        ...
        <DialogContent>
          ...
          >
            ...
            <Stack direction="row">
              <Select
                value={buttonNumber}
                onChange={(e) => setButtonNumber(e.target.value as number)}
                variant="outlined"
                sx={{ width: 70 }}
              >
                {lp.buttonNumbers.map((row, _i) =>
                  row.map((buttonNumber) => (
                    <MenuItem key={buttonNumber} value={buttonNumber}>
                      {buttonNumber}
                    </MenuItem>
                  ))
                )}
          ...
          >
            ...
            <Stack direction="row">
              ...
              <Button onClick={() => output.send(lp.command.stopText)}>
                <Stop />
              </Button>
              <Button
                onClick={() => {
                ...
                <Box sx={{ overflowY: 'auto', height: 120 }}>
                  {midiLogs.map((log, index) => (
                    <Stack key={index} direction={'row'}>
                      <Typography width={200} variant="caption">
                        {log.name}
                      </Typography>
                      <Typography width={50} variant="caption">
                        {log.note}

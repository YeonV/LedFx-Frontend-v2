...
async function checkUrl(url, expectedSubstring, serviceName) {
    ...
    http.get(url, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          if (json.name && json.name.includes(expectedSubstring)) {
            console.log(`✅ ${serviceName} is running and reachable.`)
            resolve(true)
          } else {
            console.log(
              `⚠️ ${serviceName} responded but name "${json.name}" does not contain "${expectedSubstring}".`
            )
            resolve(false)
          }
        } catch (e) {
...
async function main() {
  ...
  const frontendOk = await checkUrl(FRONTEND_URL, 'LedFx', 'Frontend')
